...
const STATUS_COLORS = {
  ...
  processing: { bg: '#e8d5f5', color: '#5b21b6' },
...
export default function AdminPage() {
  ...
  function handleField(key, value) {
    setForm(p => {
      ...
      if (key === 'name' && panel === 'add') next.slug = slugify(value)
      return next
    })
  }
    ...
    <div className="min-h-dvh bg-gray-50 font-sans">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-100 px-6 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          {['products', 'orders', 'subscribers'].map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
        ...
        {tab === 'products' && (
          <>
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={openAdd}
                className="px-4 py-2 bg-gray-900 text-white text-sm font-semibold rounded-lg hover:opacity-90 transition-opacity"
              >
                + Add Product
              </button>
            </div>
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-gray-100">
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Product</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Category</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Price</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Status</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {products.map(p => (
                      <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3.5">
                          ...
                          >
                            ...
                            <span className="text-gray-600">{p.available ? 'Available' : 'Sold out'}</span>
                          </button>
                        </td>
                        <td className="px-4 py-3.5">
          ...
          <>
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Customer</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Source</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Total</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Status</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Date</th>
                  </tr>
                </thead>
                <tbody>
                  {orders.map(order => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5">
                        <span
                          className="text-xs font-semibold px-2 py-0.5 rounded-full capitalize"
                          style={STATUS_COLORS[order.status] || { bg: '#f0f0f0', color: '#666' }}
                        >
                          {order.status}
                        </span>
                      </td>
                  ...
                  {orders.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-4 py-12 text-center text-sm text-gray-400">No orders yet.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <>
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Email</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide bg-gray-50">Signed Up</th>
                  </tr>
                </thead>
                <tbody>
                  {subscribers.map(s => (
                  ...
                  {subscribers.length === 0 && (
                    <tr>
                      <td colSpan={2} className="px-4 py-12 text-center text-sm text-gray-400">No subscribers yet.</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="p-5 flex flex-col gap-4">
              <div className="flex gap-2">
                <span
                  ...
                  style={{
                    background: selectedOrder.source === 'whatsapp' ? '#d4f7e2' : '#fce4ec',
                    color: selectedOrder.source === 'whatsapp' ? '#1a6e3c' : '#9c0f3f',
                  }}
                >
                  {selectedOrder.source || 'paystack'}
                </span>
                <span
                  className="text-xs font-semibold px-2.5 py-1 rounded-full capitalize"
                  style={STATUS_COLORS[selectedOrder.status] || { background: '#f0f0f0', color: '#666' }}
                >
        ...
        >
          <div className="bg-gray-50 rounded-2xl w-full max-w-[480px] max-h-[calc(100dvh-2rem)] overflow-y-auto shadow-2xl animate-fade-up">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 bg-gray-50 sticky top-0 z-10">
              ...
              <button
                onClick={closePanel}
                className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-300 transition-colors text-lg leading-none"
              >
                ×
              </button>
            </div>
            ...
            <div className="p-6 flex flex-col gap-4">
                ...
                {[['available', 'Available'], ['featured', 'Featured']].map(([key, label]) => (
                  <label key={key} className="flex items-center gap-2.5 cursor-pointer">
                    <div
                      ...
                      className={`relative w-10 h-6 rounded-full transition-colors ${form[key] ? 'bg-green-500' : 'bg-gray-200'}`}
                    >
              ...
              {msg && (
                <p className={`text-sm font-medium text-center ${msg.startsWith('Error') ? 'text-red-500' : 'text-green-600'}`}>
                  {msg}
                </p>
              )}
              ...
              <div className="flex gap-3 pt-1">
                <button
                  onClick={closePanel}
                  className="flex-1 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="flex-[2] py-3 bg-gray-900 text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-40"
                >
