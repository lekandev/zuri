...
export default function ProductDetailClient({ product }: ProductDetailClientProps) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      <Link href="/store" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-900 transition-colors mb-8">
        ← Back to shop
      </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 items-start">
        ...
        <div className="space-y-3">
          <div className="aspect-[4/5] bg-gray-100 rounded-2xl overflow-hidden">
            {images.length > 0 ? (
              <img src={images[selectedImage]} alt={product.name} className="w-full h-full object-cover transition-opacity duration-200" />
            ) : (
            ...
            <div className="flex gap-2 overflow-x-auto">
              {images.map((img, i) => (
                <button
                  ...
                  className={`w-14 h-[72px] rounded-lg overflow-hidden shrink-0 border-2 transition-colors ${selectedImage === i ? 'border-gray-900' : 'border-transparent'}`}
                >
        ...
        <div className="flex flex-col gap-6 pt-2">
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900">{currencySymbol}{Number(product.price).toLocaleString()}</p>
          </div>
          ...
          <div className="flex flex-col gap-3">
            {product.available ? (
              <>
                ...
                {WHATSAPP && (
                  <button onClick={handleWhatsApp} className="w-full py-3.5 bg-[#25D366] text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-2">
                    <WhatsAppIcon />
                    Order via WhatsApp
                  </button>
                )}
          ...
          <div className="border-t border-gray-100 pt-5 space-y-1">
            <details className="group">
              <summary className="flex justify-between items-center text-sm font-semibold cursor-pointer py-2 list-none">
                Care & Details <span className="text-gray-400 group-open:rotate-45 transition-transform">+</span>
              </summary>
            ...
            <details className="group">
              <summary className="flex justify-between items-center text-sm font-semibold cursor-pointer py-2 list-none">
                Shipping <span className="text-gray-400 group-open:rotate-45 transition-transform">+</span>
              </summary>
              <div className="pt-2 pb-3 text-sm text-gray-500 leading-loose space-y-0.5">
