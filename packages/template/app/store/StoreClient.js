    ...
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      {hasFilters && (
        <div className="flex gap-2 flex-wrap pb-6 overflow-x-auto">
          {categories.map(c => (
            <button key={c} className={pillClass(category === c)} onClick={() => setCategory(c)}>
              {c}
            </button>
          ))}
          ...
          {sizes.map(s => (
            <button key={s} className={pillClass(size === s)} onClick={() => setSize(s)}>
              {s}
            </button>
          ))}
          ...
          <button
            className={pillClass(showAvailable)}
