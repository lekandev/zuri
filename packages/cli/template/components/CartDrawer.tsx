...
function Field({ label, ...inputProps }: FieldProps) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        className="bg-white border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 outline-none focus:border-gray-400 transition-colors w-full"
        {...inputProps}
      />
    </div>
  )
...
export default function CartDrawer() {
  ...
  function setField(key: keyof Customer) {
  ...
  async function handleWhatsAppOrder() {
    const lines = items
      .map(i =>
        `• ${i.name}${i.selectedSize !== 'One size' ? ` (${i.selectedSize})` : ''} x${i.quantity} — ${currencySymbol}${(i.price * i.quantity).toLocaleString()}`
      )
      .join('\n')
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {step === 'success' ? (
            ...
            <div className="flex flex-col gap-4">
              ...
              <Field label="Name" type="text" placeholder="Your full name" value={customer.name} onChange={setField('name')} />
              <Field label="Email" type="email" placeholder="your@email.com" value={customer.email} onChange={setField('email')} />
              <Field label="Phone (optional)" type="tel" placeholder="+234..." value={customer.phone} onChange={setField('phone')} />
              <Field label="Delivery address" type="text" placeholder="Street address" value={customer.address} onChange={setField('address')} />
              <div className="grid grid-cols-2 gap-3">
                <Field label="City" type="text" placeholder="City" value={customer.city} onChange={setField('city')} />
                <Field label="State" type="text" placeholder="State" value={customer.state} onChange={setField('state')} />
              </div>
